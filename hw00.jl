...
ex_1_1 = md"""
If $a = \sqrt{x}$ then $a \times a = x$ then $a = \frac{x}{a}$

If $a \lt \sqrt{x}$ then $a \times a \lt x$ then $a \lt \frac{x}{a}$

So larger $a$ would be closer to $\sqrt{x}$ until $a > \frac{x}{a}$, when $a \times a \gt x$ so $a \gt \sqrt{x}$ so larger $a$ would be further from $\sqrt{x}$.

So the square root must either be larger than $a$ but not as large as $\frac{x}{a}$, or smaller than $a$ but not as small as $\frac{x}{a}$.  Either way, the square root is between $a$ and $\frac{x}{a}$. 
""" 
...
function newton_sqrt(x, error_margin=0.01, a=x / 2)
	a_old = x
	a_new = a
	while abs(a_old - a_new) > error_margin
		a_old = a_new
		a_new = (x / a_old + a_old) / 2
	end
	return a_new
end
