...
# ╔═╡ 3c41bd50-6351-11eb-2a27-4563a9de84f4
@code_native 1+1
...
begin
	myclamp(x, lo, hi) = ifelse(x > hi, hi, ifelse(x < lo, lo, x))
	myclamp(x) = myclamp(x, 0, 1)
	...
	function noisify(color::AbstractRGB, s)
		mapc(x -> noisify(x, s), color)
	end
	
	function noisify(image::AbstractMatrix, s)
		map(x -> noisify(x, s), image)
	end
end

# ╔═╡ 6c28c6de-5ea2-11eb-0900-15cdd3dff7bf
rand()
...
philip_inverted = map(invert, philip)
...
# ╔═╡ 14ac4538-5f5c-11eb-3770-a7eadeb5aa0b
...
function blur_1D(v, l)
	[mean(extend(v, (-l+i):(l+i))) for i in 1:length(v)]
end
...
# ╔═╡ 38b03eec-5f59-11eb-017a-776f3bdb6eb0
begin
	l_box_slider = @bind l_box html"<input type=range>"
	
	md"Window size: $(l_box_slider)"
end

# ╔═╡ b2bd9880-5f5f-11eb-28e6-63bcae98944b
md"The window size is $(l_box)."

# ╔═╡ 17e5d1a0-5f5d-11eb-0310-2d2175776859
...
function convolve_vector(v, k)
	ℓk = (length(k) - 1) ÷ 2
	lv = length(v)
	[sum([v[myclamp(i-n, 1, lv)] * k[n+ℓk+1] for n in -ℓk:ℓk]) for i in 1:lv]
end
...
function gaussian_kernel(n)
	k = (n - 1) ÷ 2
	g = [(1/sqrt(2π))exp((-i^2)/2) for i in -k:k]
	g/sum(g)
end
...
begin
	gaussian_kernel_size_1D_slider = @bind gaussian_kernel_size_1D html"<input type=range>"
	
	md"Gaussian kernal size: $(gaussian_kernel_size_1D_slider)"
end

# ╔═╡ 8d24d8de-6030-11eb-033f-436695a11c85
gaussian_kernel(gaussian_kernel_size_1D)

# ╔═╡ 71e7e262-602d-11eb-0073-ada1618d342f
md"$(gaussian_kernel_size_1D)"
...
function extend_mat(M::AbstractMatrix, i, j)
	...
	i_clamped = myclamp(i, 1, num_rows)
	j_clamped = myclamp(j, 1, num_columns)
	M[i_clamped, j_clamped]
end
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	Mm, Mn = size(M) # size of the image
	Km, Kn = (size(K) .- 1) .÷ 2 # nearest odd radius of K
	
	# Multiply the pixels around the position by each element of K
	conv_single((x, y)) = sum([extend_mat(M, i, j) for i in -Km+x:Km+x, j in -Kn+y:Kn+y] .* K)
		
	# for each pixel position in the image
	conv_single.([(x, y) for x in 1:Mm, y in 1:Mn])
end
...
function with_gaussian_blur(image)
    r = 5 # arbitrary
    σ = 5 # arbitrary
    temp = [exp(-(x^2+y^2)/2σ^2)/(2π*σ^2) for x=-r:r,y=-r:r]
    G = [temp[x,y]/sum(temp) for x=1:(2r+1),y=1:(2r+1)]
    convolve_image(image, G)
end
...
function with_sobel_edge_detect(image)
	x = transpose([1  2  1]) * [1  0 -1]
	y = transpose([1  0 -1]) * [1  2  1]
	G_x = convolve_image(image, x)
	G_y = convolve_image(image, y)
